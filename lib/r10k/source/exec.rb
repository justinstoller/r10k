...
class R10K::Source::Exec < R10K::Source::Hash
  ...
  def initialize(name, basedir, options = {})
    unless @command = options[:command]
      raise ConfigError, 'Environment source %{name} missing required parameter: command' % {name: name}
    end
  ...
  def run_environments_command
    ...
    begin
      ...
      begin
        ...
        raise R10K::Error, "Error parsing command output for exec source %{name}:\n" \
    ...
    unless R10K::Source::Hash.valid_environments_hash?(environments)
      raise R10K::Error, "Environment source %{name} command %{cmd} did not return valid environment data.\n" \
