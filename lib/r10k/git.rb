  ...
  module Git
          ...
          :on_set  => proc do
            [:ssh, :https].each do |transport|
              if !::Rugged.features.include?(transport)
                logger.warn "Rugged has been compiled without support for %{transport}; Git repositories will not be reachable via %{transport}." % {transport: transport}
              end
            end
          end
        }
    ...
    def self.provider=(name)
      ...
      if attrs.nil?
        ...
        raise R10K::Error, "No Git provider named '%{name}'." % {name: name}
      end
      if !R10K::Features.available?(attrs[:feature])
        @provider = NULL_PROVIDER
        raise R10K::Error, "Git provider '%{name}' is not functional." % {name: name}
      end
      if attrs[:on_set]
        attrs[:on_set].call
      end
      ...
      logger.debug1 { "Setting Git provider to %{provider}" % {provider: @provider.name} }
    end
    ...
    def self.provider
      ...
      when UNSET_PROVIDER
        ...
        logger.debug1 { "Setting Git provider to default provider %{name}" % {name: default_name} }
      end
