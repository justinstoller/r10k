  ...
  module Forge
    ...
    class ModuleRelease
      ...
      def unpack(target_dir)
        logger.debug1 "Unpacking %{tarball_cache_path} to %{target_dir} (with tmpdir %{tmp_path})" % {tarball_cache_path: tarball_cache_path, target_dir: target_dir, tmp_path: unpack_path}
        file_lists = PuppetForge::Unpacker.unpack(tarball_cache_path.to_s, target_dir.to_s, unpack_path.to_s)
        logger.debug2 "Valid files unpacked: %{valid_files}" % {valid_files: file_lists[:valid]}
        if !file_lists[:invalid].empty?
          logger.debug1 "These files existed in the module's tar file, but are invalid filetypes and were not unpacked: %{invalid_files}" % {invalid_files: file_lists[:invalid]}
        end
        if !file_lists[:symlinks].empty?
          logger.warn "Symlinks are unsupported and were not unpacked from the module tarball. %{release_slug} contained these ignored symlinks: %{symlinks}" % {release_slug: @forge_release.slug, symlinks: file_lists[:symlinks]}
        end
      end
